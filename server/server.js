...
const API_KEYS = {
    tmdb: process.env.TMDB_API_KEY || '84ca41a4c565bdcf9b36eca75de5c989',
...
const server = http.createServer(async (req, res) => {
    ...
    try {
        ...
        if (pathname === '/streams/resolve' && method === 'POST') {
            ...
            const streamUrl = await resolveStream(tmdbId, mediaType, title, season, episode);
...
async function resolveStream(tmdbId, mediaType, title, season, episode) {
    ...
    try {
        // Step 1: Search torrents via Torrentio
        const torrentioUrl = mediaType === 'movie'
            ? `https://torrentio.strem.fun/stream/movie/${tmdbId}.json`
            : `https://torrentio.strem.fun/stream/series/${tmdbId}:${season}:${episode}.json`;
